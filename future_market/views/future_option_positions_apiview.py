...
# COVERED_CALL ##########################################
def get_low_risk_covered_call(strategy_key, table):
...
def get_high_risk_covered_call(strategy_key, table):
    strategy_result = get_strategy_result_from_mongo(strategy_key, table)
    if not strategy_result.empty:
        strategy_result = strategy_result[
            (
...
# CONVERSION ############################################
def get_no_risk_conversion(strategy_key, table):
    strategy_result = get_strategy_result_from_mongo(strategy_key, table)
    if not strategy_result.empty:
...
# LONG_CALL ############################################
def get_high_risk_long_call(strategy_key, table):
    strategy_result = get_strategy_result_from_mongo(strategy_key, table)
    strategy_result = strategy_result.to_dict(orient="records")
    return strategy_result
...
# SHORT_CALL ###########################################
def get_low_risk_short_call(strategy_key, table):
    strategy_result = get_strategy_result_from_mongo(strategy_key, table)
    if not strategy_result.empty:
        strategy_result = strategy_result[
...
def get_high_risk_short_call(strategy_key, table):
    strategy_result = get_strategy_result_from_mongo(strategy_key, table)
    if not strategy_result.empty:
        strategy_result = strategy_result[
            strategy_result["base_equity_last_price"] > strategy_result["strike_price"]
...
# LONG_PUT #############################################
def get_high_risk_long_put(strategy_key, table):
    strategy_result = get_strategy_result_from_mongo(strategy_key, table)
    strategy_result = strategy_result.to_dict(orient="records")
    return strategy_result
...
# SHORT_PUT ############################################
def get_high_risk_short_put(strategy_key, table):
    strategy_result = get_strategy_result_from_mongo(strategy_key, table)
    if not strategy_result.empty:
        strategy_result = strategy_result[
            strategy_result["strike_price"] > strategy_result["base_equity_last_price"]
...
# LONG_STRADDLE #######################################
def get_high_risk_long_straddle(strategy_key, table):
    strategy_result = get_strategy_result_from_mongo(strategy_key, table)
    strategy_result = strategy_result.to_dict(orient="records")
...
# SHORT_STRADDLE #######################################
def get_high_risk_short_straddle(strategy_key, table):
    strategy_result = get_strategy_result_from_mongo(strategy_key, table)
    strategy_result = strategy_result.to_dict(orient="records")
...
# BULL_CALL_SPREAD ############################################
def get_low_risk_bull_call_spread(strategy_key, table):
    strategy_result = get_strategy_result_from_mongo(strategy_key, table)
    if not strategy_result.empty:
        strategy_result = strategy_result[
...
def get_high_risk_bull_call_spread(strategy_key, table):
    strategy_result = get_strategy_result_from_mongo(strategy_key, table)
    if not strategy_result.empty:
        strategy_result = strategy_result[
...
# BEAR_CALL_SPREAD #######################################
def get_high_risk_bear_call_spread(strategy_key, table):
    strategy_result = get_strategy_result_from_mongo(strategy_key, table)
    strategy_result = strategy_result.to_dict(orient="records")
...
# BULL_PUT_SPREAD #######################################
def get_low_risk_bull_put_spread(strategy_key, table):
    strategy_result = get_strategy_result_from_mongo(strategy_key, table)
    strategy_result = strategy_result.to_dict(orient="records")
...
def get_high_risk_bull_put_spread(strategy_key, table):
    strategy_result = get_strategy_result_from_mongo(strategy_key, table)
    if not strategy_result.empty:
        strategy_result = strategy_result[
...
# BEAR_PUT_SPREAD #######################################
def get_low_risk_bear_put_spread(strategy_key, table):
    strategy_result = get_strategy_result_from_mongo(strategy_key, table)
    if not strategy_result.empty:
        strategy_result = strategy_result[
            strategy_result["put_sell_strike"]
...
def get_high_risk_bear_put_spread(strategy_key, table):
    strategy_result = get_strategy_result_from_mongo(strategy_key, table)
    strategy_result = strategy_result.to_dict(orient="records")
...
# LONG_STRANGLE ############################################
def get_low_risk_long_strangle(strategy_key, table):
    strategy_result = get_strategy_result_from_mongo(strategy_key, table)
    if not strategy_result.empty:
        strategy_result = strategy_result[
            strategy_result["call_buy_strike"]
...
def get_high_risk_long_strangle(strategy_key, table):
    strategy_result = get_strategy_result_from_mongo(strategy_key, table)
    if not strategy_result.empty:
        strategy_result = strategy_result[
            strategy_result["call_buy_strike"]
...
# SHORT_STRANGLE #######################################
def get_high_risk_short_strangle(strategy_key, table):
    strategy_result = get_strategy_result_from_mongo(strategy_key, table)
    strategy_result = strategy_result.to_dict(orient="records")
...
# LONG_BUTTERFLY #######################################
def get_high_risk_long_butterfly(strategy_key, table):
    strategy_result = get_strategy_result_from_mongo(strategy_key, table)
    strategy_result = strategy_result.to_dict(orient="records")
...
# SHORT_BUTTERFLY #######################################
def get_high_risk_short_butterfly(strategy_key, table):
    strategy_result = get_strategy_result_from_mongo(strategy_key, table)
    strategy_result = strategy_result.to_dict(orient="records")
...
# COLLAR ############################################
def get_low_risk_collar(strategy_key, table):
    strategy_result = get_strategy_result_from_mongo(strategy_key, table)
    if not strategy_result.empty:
        strategy_result = strategy_result[
            strategy_result["call_strike_low"]
            < strategy_result["base_equity_last_price"]
        ]
    strategy_result = strategy_result.to_dict(orient="records")
...
def get_high_risk_collar(strategy_key, table):
    strategy_result = get_strategy_result_from_mongo(strategy_key, table)
    if not strategy_result.empty:
        strategy_result = strategy_result[
