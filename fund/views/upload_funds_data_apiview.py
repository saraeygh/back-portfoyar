...
from fund.utils import check_funds_uploaded_data_thread


@authentication_classes([TokenAuthentication])
@permission_classes([IsAdminUser])
class CheckUploadDataAPIView(APIView):
    def post(self, request):
        excel_file = request.FILES.get("funds_data")
        ...
        #######################################################################
        # check_funds_uploaded_data_thread(file_name)
        #######################################################################

        #######################################################################
        check_funds_uploaded_data_thread_thread = threading.Thread(
            target=check_funds_uploaded_data_thread, args=(file_name,)
        )
        check_funds_uploaded_data_thread_thread.start()
        #######################################################################
