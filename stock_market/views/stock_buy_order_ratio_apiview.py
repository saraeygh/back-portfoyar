...
class StockBuyOrderRatioAPIView(APIView):
    def get(self, request):
        ...
        results = results.sort_values(by="buy_ratio", ascending=False)
        ...
        if table and table == "summary":
            results = SummaryBuyOrderRatioSerailizer(results, many=True)
        else:
            results = BuyOrderRatioSerailizer(results, many=True)
