...
from favorite.serializers import (
    DomesticAddFavoriteRatioChartByCategorySerailizer,
...
class DomesticFavoriteRatioChartByCategoryAPIView(APIView):
    def get(self, request):
        user = get_object_or_404(User, id=request.user.id)

        domestic_favorite_ratio_charts = (
            user.domestic_favorite_ratio_chart_by_category.all()
        )

        domestic_favorite_ratio_charts_srz = (
            DomesticGetFavoriteRatioChartByCategorySerailizer(
                domestic_favorite_ratio_charts, many=True
            )
        )

        return Response(
            domestic_favorite_ratio_charts_srz.data,
    ...
    def post(self, request):
        ...
        validated_data_dict_1 = domestic_validate_favorite_by_category(
        ...
        validated_data_dict_2 = domestic_validate_favorite_by_category(
        ...
        if validated_data_dict_1 and validated_data_dict_2:
            ...
            favorite_exists = DomesticFavoriteRatioChartByCategory.objects.filter(
    ...
    def delete(self, request, *args, **kwargs):
        favorite_ratio_chart_id = kwargs.get("favorite_id")
