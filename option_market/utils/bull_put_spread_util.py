...
def bull_put_spread():
    ...
    for end_date_option in tqdm(
        distinct_end_date_options, desc="bull_put_spread", ncols=10
    ):
        ...
        if cartesians:
            for _, row in cartesians[0].iterrows():
                ...
                high_premium = float(high_premium.get("put_best_buy_price").iloc[0])
                add_option.add_put_sell(strike=high_strike, premium=high_premium)
                ...
                document = {
                    ...
                    "put_sell_notional_value": sell_row.get("put_notional_value") / RIAL_TO_BILLION_TOMAN,
    ...
    print(f"bull_put_spread, {len(result)} records.")
