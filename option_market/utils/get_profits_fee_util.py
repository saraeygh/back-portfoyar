...
def add_fee(strike, premium, action, option_type):
...
def get_fee_percent(profits, strike_sum=0, premium_sum=0, net_pay=1):
    strike_fee = strike_sum * OPTION_SETTLEMENT_FEE
    premium_fee = premium_sum * OPTION_TRADE_FEE

    profits["fee"] = (sum([strike_fee, premium_fee]) / net_pay) * 100
